...
Para compilar el programa desde la terminal y mover la aplicación al escritorio, ejecuta los siguientes comandos:

```bash
git clone https://github.com/srimanachanta/Stasis.git
cd Stasis
xcodebuild -scheme stasis -configuration Release CONFIGURATION_BUILD_DIR="$(pwd)/build"
cp -R build/Stasis.app ~/Desktop/
```

Opcionalmente, puedes abrir el proyecto en Xcode y compilarlo desde ahí:
