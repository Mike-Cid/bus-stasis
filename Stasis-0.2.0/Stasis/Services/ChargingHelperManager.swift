...
class ChargingHelperManager {
    ...
    func install() throws {
        ...
        // Force a clean re-registration so macOS refreshes LaunchDaemon metadata
        // (path, checksum and launch constraints) after app updates.
        if service.status == .enabled {
            do {
                try service.unregister()
            } catch {
                logger.warning("Previous charging helper unregister failed: \(error.localizedDescription)")
            }
        }
