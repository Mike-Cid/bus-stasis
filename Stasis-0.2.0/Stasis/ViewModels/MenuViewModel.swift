...
class MenuViewModel {
    ...
    func toggleWorkWithAC() {
        chargeManager.toggleWorkWithAC()
    }
    ...
    private func updateWorkWithACStatus(
        batteryPower: Double,
        isCharging: Bool,
        ...
        guard adapterConnected else {
            workWithACStatusText = "Unavailable (Adapter disconnected)"
            workWithACBatteryAssistDetected = false
            workWithACChargingDetected = false
            return
        }

        let chargingThreshold = 0.5
        let assistThreshold = -0.5
        let chargingDetected = isCharging || batteryPower > chargingThreshold
        let assistDetected = batteryPower < assistThreshold

        workWithACChargingDetected = chargingDetected
        workWithACBatteryAssistDetected = assistDetected

        if chargingDetected {
            workWithACStatusText = "Warning: Battery charging detected"
        } else if assistDetected {
            workWithACStatusText = "AC limited: Battery assisting system load"
        } else {
            workWithACStatusText = "AC only: Battery idle"
        }
    }
