...
class ChargeManager {
    ...
    private(set) var workWithACActive = Defaults[.workWithACActive] {
        didSet {
            Defaults[.workWithACActive] = workWithACActive
        }
    }
    ...
    private func evaluate(controlState: BatteryControlState) {
        ...
        if workWithACActive {
            // Keep adapter enabled while preventing any intentional charging.
            desiredCharging = false
            desiredAdapter = true
            chargingStateReason = "Work with AC is active"
            if Defaults[.manageMagSafeLED] {
                desiredLED = .green
            }
        }
